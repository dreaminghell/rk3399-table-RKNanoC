...
**/
    ...
    /*int (*compass_en)(struct inv_mpu_iio_s *, struct iio_buffer *ring, bool on); **/
    /*int (*quaternion_en)(struct inv_mpu_iio_s *, struct iio_buffer *ring, bool on); **/
    /*int (*gyro_en)(struct inv_mpu_iio_s *, struct iio_buffer *ring, bool on); **/
    /*int (*accl_en)(struct inv_mpu_iio_s *, struct iio_buffer *ring, bool on); **/
    int (*init_config)(struct inv_mpu_iio_s *st);
    void (*setup_reg)(struct inv_reg_map_s *reg);

    int (*plat_read)(struct inv_mpu_iio_s *st, uint8 reg, int len, uint8 *data);
    int (*plat_single_write)(struct inv_mpu_iio_s *st, uint8 reg, uint8 data);
    int (*secondary_read)(struct inv_mpu_iio_s *st, uint8 reg, int len, uint8 *data);
    int (*secondary_write)(struct inv_mpu_iio_s *st, uint8 reg, uint8 data);
    int (*memory_write)(struct inv_mpu_iio_s *st, uint8 mpu_addr, uint16 mem_addr, uint32 len, uint8 const *data);
    int (*memory_read)(struct inv_mpu_iio_s *st, uint8 mpu_addr, uint16 mem_addr, uint32 len, uint8 *data);
#if 0
