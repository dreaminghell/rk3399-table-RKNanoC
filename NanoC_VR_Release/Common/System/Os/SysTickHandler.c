...
_ATTR_SYS_CODE_
UINT32 SystickTimerReStart(SYSTICK_LIST *pSystick)
{
    if(SystickTimerCheck(pSystick))
    {
        SystickTimerStopRaw(&SysTickHead, pSystick);
...
/*
--------------------------------------------------------------------------------
  Function name : void SystickDelaynS(uint32 ns)
  Author        :
  Description   : delay "ns" with systick
...
{
    SysTickDly = nvic->SysTick.Value;
}

void SystickDelaynS(uint32 ns)
{
    UINT32 pclk, ulPeriod, DestVal;
    uint32 CurVal  = SysTickDly; //nvic->SysTick.Value;
    uint32 Reload  = nvic->SysTick.Reload + 1;
    uint32 i,temp;

    pclk = GetPllSysFreq();
    ulPeriod = ns * pclk / 1000;

    if (CurVal <= ulPeriod)
    {
        SysTickDly = Reload + CurVal - ulPeriod;
        for (i = 0; i < ulPeriod * 10; i++)
        {
            temp = nvic->SysTick.Value;
            if ((CurVal < temp) && (temp <= SysTickDly))
            {
                break;
            }
        }
