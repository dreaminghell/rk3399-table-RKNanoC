...
{
    void *pWinArg;
	uint32 current_times = 0;
	uint32 start_times = GetSysTick();
    ...
    while(1)
    {
        *pWin = TaskInit(&pWinArg);//task adjust,call new task code,rturn main window pointer.
        ...
        while(1)
        {
			...
			if(gSysConfig.StartTP == 0)
			{
				current_times = GetSysTick();
				if(current_times-start_times >= 280)
				{
					LCD_BL_On();
					gSysConfig.StartTP = 1;
					gSysConfig.UsbSensor = 1;
				}
			} else {
				start_times = SysTickCounter;
			}
        }
        //end the main window.
        WinDestroy(*pWin);
